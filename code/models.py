...
class HAN(nn.Module):
    def __init__(self, config):
        ...
        self.word_context_weights = nn.Parameter(
            torch.rand(2*self.word_hidden_dim, 1))
        self.word_context_weights.data.uniform_(-0.25, 0.25)
        self.word_attn_gru = nn.GRU(
            self.embed_dim, self.word_hidden_dim, bidirectional=True)
        self.word_lin_projection = nn.Sequential(
            nn.Linear(2*self.word_hidden_dim, 2*self.word_hidden_dim), nn.Tanh())
        self.word_attn_wts = nn.Softmax()

        # Sentence attention
        self.sent_hidden_dim = config['sent_gru_dim']
        self.sent_context_weights = nn.Parameter(
            torch.rand(2*self.sent_hidden_dim, 1))
        self.sent_context_weights.data.uniform_(-0.1, 0.1)
        self.sentence_attn_gru = nn.GRU(
            2*self.word_hidden_dim, self.sent_hidden_dim, bidirectional=True, batch_first=False)
        self.sent_lin_projection = nn.Sequential(
            nn.Linear(2*self.sent_hidden_dim, 2*self.sent_hidden_dim), nn.Tanh())
    ...
    def forward(self, inp, embedding, length):
        ...
        for i in range(num_sents):
            ...
            out = torch.mul(all_states_words.permute(
                2, 0, 1), out.transpose(1, 0))
            word_attn_outs = torch.sum(out, dim=1).transpose(1, 0).unsqueeze(0)
            if sent_representations is None:
                sent_representations = word_attn_outs
            else:
                sent_representations = torch.cat(
                    (sent_representations, word_attn_outs), dim=0)
