...
def prepare_training(config):
    ...
    train_sorted, val_sorted, test_sorted, train_sorted_idx, val_sorted_idx, test_sorted_idx = get_distributions(
        config, train_split, val_split, test_split)
    ...
    zero = sorted([val_sorted_idx[i]
                   for i, x in enumerate(val_sorted) if x < 1e-3])
...
def get_distributions(config, train_split, val_split, test_split):
    ...
    train_distrib, val_distrib, test_distrib = np.zeros((len(train_split), classes)), np.zeros(
        (len(val_split), classes)), np.zeros((len(test_split), classes))
